...
class InfiniteGridMenu {
  ...
  constructor(
    ...
    onInit?: (sketch: InfiniteGridMenu) => void
  ) {
    this.init(onInit);
  }

  private init(onInit?: (sketch: InfiniteGridMenu) => void): void {
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  const [atlasImage, setAtlasImage] = useState<string>('');
  ...
  useEffect(() => {
    ...
    if (canvas) {
      sketch = new InfiniteGridMenu(canvas, items, handleActiveItem, setIsMoving, sk => {
        // Create and store the atlas image for modal
        createAtlasImage(items).then(imageUrl => {
  ...
  // Function to create atlas image
  const createAtlasImage = async (items: MenuItem[]): Promise<string> => {
  ...
  // Function to extract specific cell from atlas
  const getImageFromAtlas = (): Promise<string> => {
    return new Promise((resolve) => {
      if (!atlasImage || activeItemIndex === null || !items[activeItemIndex]) {
        resolve(activeItem?.image || '');
        return;
      }
      
      const itemCount = Math.max(1, items.length);
      const atlasSize = Math.ceil(Math.sqrt(itemCount));
      const cellX = activeItemIndex % atlasSize;
      const cellY = Math.floor(activeItemIndex / atlasSize);
      
      // Create a canvas to extract the specific cell
      const canvas = document.createElement('canvas');
      const ctx = canvas.getContext('2d')!;
      canvas.width = 512;
      canvas.height = 512;
      ...
      img.onerror = () => resolve(activeItem?.image || '');
    });
  };
  ...
  const handleButtonClick = async () => {
    ...
    // Use the real product data directly from the active item
    const extractedImage = await getImageFromAtlas();
    
    // Create product object for the modal using the existing data
    const product = {
      ...activeItem,
      id: activeItem._id || activeItem.id || activeItem.productId || `infinite-${activeItemIndex}`,
      _id: activeItem._id || activeItem.id || activeItem.productId || `infinite-${activeItemIndex}`,
      image: extractedImage || activeItem.image,
      images: activeItem.images || [extractedImage || activeItem.image],
      src: extractedImage || activeItem.src || activeItem.image,
      price: activeItem.price || 99.99,
      displayPrice: activeItem.displayPrice || `$${(activeItem.price || 99.99).toFixed(2)}`,
      categories: activeItem.category ? [{
        id: `category-${activeItemIndex}`,
        _id: `category-${activeItemIndex}`,
        title: activeItem.category,
        image: ''
      }] : [],
      ratingDisplay: activeItem.ratingDisplay || (activeItem.averageRating ? activeItem.averageRating.toFixed(1) : 'No ratings'),
    };
  ...
  return (
    <div className='infinit'>
      <div className="infinite-top">
        <span className='theme-gradient'> 
          <h2>New Collections</h2>
        </span>
        <p>Click on the canvas below and drag!</p>
      </div>
      ...
      {isModalOpen && modalProduct && (
        <ProductDetailModal
          ...
          colorScheme="purple" // Using purple theme for InfiniteMenu
        />
      )}
    </div>
  );
